...
SECCIONES = {
    ...
    # Nota: Corregido 8PSG.DE a 8PSD.DE para que coincida con el diccionario de traducción
...
# Recopilar todos los activos únicos
...
def descargar_datos_seguro():
    ...
    for i, ticker in enumerate(TODOS_ACTIVOS):
        ...
        try:
            ...
            # Descargamos desde 2005 para tener histórico suficiente para los backtests largos
            historial = ticker_obj.history(start="2005-01-01")
            if not historial.empty:
                df_result[ticker] = historial['Close']
            time.sleep(0.3) # Pausa táctica
    ...
    time.sleep(1) # Pequeña pausa visual antes de limpiar
    status.empty()
    bar.empty()
...
if st.button("🚀 Ejecutar Backtest"):
    ...
    if not precios.empty:
        ...
        # Filtrar datos desde que todos los activos tienen cotización
        ...
        for i in range(len(dias_reb)):
            ...
            for nom, conf in SECCIONES.items():
                ventana = precios.loc[:f_ini, conf['activos']].tail(lookback + 1)
                ...
                validos = [t for t in conf['activos'] if rent_10d[t] > 0 and precios.loc[f_ini, t] > sma_vals.loc[f_ini, t]]
                ganador = sharpe[validos].idxmax() if validos else "BIL"
        ...
        df_res = pd.DataFrame({
            ...
            'Benchmark': (1 + retornos_diarios.loc[ret_est.index, BENCHMARK]).cumprod()
        })
        ...
        # Gráficos de evolución (quitamos la zona horaria para evitar errores de Streamlit)
        ...
        for idx, (nom, conf) in enumerate(SECCIONES.items()):
            v_h = precios[conf['activos']].tail(lookback + 1)
            r_h = (v_h.iloc[-1] / v_h.iloc[0]) - 1
            val_h = [t for t in conf['activos'] if r_h[t] > 0 and precios.loc[hoy, t] > sma_vals.loc[hoy, t]]
        ...
        # Botón para descargar a Excel/Sheets
